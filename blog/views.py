...
from .form import BlogPost  # forms.py의 BlogForm을 가져옴.
...
def home(request):
    ...
    # blogs라는 키값을 blogs라는 value값으로 사용
...
def create(request):
    if request.method == 'POST':  # 입력된 내용을 처리(POST)
        form = BlogPost(request.POST)  # POST방식으로 들어온 data를 form에 넣어줌.
        if form.is_valid():
...
def edit(request, blog_id):
    ...
    if request.method == 'POST':  # post방식인지 판단
        ...
        if form.is_valid():  # 입력값 체크
            blog = form.save(commit=False)  # So, blog객체를 가져옴:O & 바로 save:X
            blog.pub_date = timezone.datetime.now()  # 현재 날짜&시간(form에서 입력 받지 않았음)
